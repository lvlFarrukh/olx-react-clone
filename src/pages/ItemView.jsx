...
class ItemView extends React.Component {
  ...
  componentDidMount() {
    setTimeout(() => {
      this.setState({ checkBoxCat: this?.state?.filterAdds[0]?.catagory });
    }, 4000);
  ...
  componentDidUpdate() {
    const seachingAdds = () => {
      let filerAdds = this?.props?.location?.state?.allAdds.filter((v) => {
        return (
          (v?.City?.toLowerCase() ===
            (this?.props?.searchParameter?.location === null
              ? v?.City?.toLowerCase()
              : this?.props?.searchParameter?.location?.toLowerCase()) ||
            v?.state?.toLowerCase() ===
              this?.props?.searchParameter?.location?.toLowerCase()) &&
          (v?.catagory?.toLowerCase() ===
            this?.props?.searchParameter?.search?.toLowerCase() ||
            v?.title
              ?.toLowerCase()
              ?.includes(this?.props?.searchParameter?.search?.toLowerCase()))
        );
      });
      ...
      if (this?.props?.subFilters[0]?.toLowerCase() === "company") {
        filtedAdds = this?.props?.adds?.filter((v) => {
          return (
            ...
            this?.props?.subFilters[1]?.toLowerCase()
          );
        });
      ...
      <div>
        ...
        <section>
          <div className="container-fluid main-div-VI">
            <div className="row">
              <p className="navi-option">
                {" "}
                Home / Catagory / sub-Catagory / Location{" "}
              </p>
            </div>

            <div className="row title-IV">
              <h3>{`${
                this?.props?.location?.state?.search?.charAt(0)?.toUpperCase() +
                this?.props?.location?.state?.search?.slice(1)
              } in ${
            ...
            <div className="row">
              <div className="col-3 cate-div-VI">
                ...
                <SelectorCategory
                  attr={{ type: "Condition", options: ["New", "Used"] }}
                />
              ...
              <div className="col-9">
                ...
                <div className="row">
                  {this?.props?.productView === 0
                    ? this?.state?.filterAdds?.map((v, i) => {
                        return <SequenceView key={i} attr={v} />;
                      })
                    : this?.props?.productView === 1
                    ? this?.state?.filterAdds?.map((v, i) => {
                        return <GridItemView key={i} attr={v} />;
                      })
                    : this?.state?.filterAdds?.map((v, i) => {
                        return <TitleViewCart key={i} attr={v} />;
                      })}
                </div>
              </div>
            </div>
...
const mapDispatchToProps = (dispatch) => ({
  changeHandleSearch: () => {
    dispatch(changeHandleSearch());
  },
  subFilter: (obj) => {
    dispatch(subFilter(obj));
  },
});
